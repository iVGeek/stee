...
4. `POST /api/bookings/:id/verify` verifies the reference against Paystack before marking the booking **confirmed** and sending emails.
...
The floating button, hero CTA and "Pay later on WhatsApp" flow all build `https://wa.me/<WA_NUMBER>?text=<message>` links. The booking path prefills the chat with the client's session details and booking reference.
...
3. Render reads `render.yaml`, creates the web service, runs `npm install && npm run build`, and starts `npm start`.
...
> **Note on data storage:** bookings and feedback are stored in JSON files (`data/`). This is simple and free, but on Render's free tier the filesystem is reset whenever the service restarts or redeploys. For a production practice you'll want a database. The `src/lib/store.ts` module is the single place to swap in Postgres/MongoDB/Supabase later — the routes never talk to storage directly.
...
| `POST` | `/api/bookings/:id/contact` | Save booking and get a prefilled WhatsApp link |
